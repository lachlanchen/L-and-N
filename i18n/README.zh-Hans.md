...
[苹果 App Store — 0.99 美元](https://apps.apple.com/us/app/l-n-speech-practice/id6808872450) · [Google Play — 免费下载，含应用内购买](https://play.google.com/store/apps/details?id=art.lazying.landn) · [官方链接](../docs/public-links.md)
...
声学特征、分数、进度和校准均在本地运行。在 iOS 上，同一条原生音频引擎数据流同时供波形、本地声学分析和系统语音识别使用，应用内部不会再争抢麦克风。托管的 PWA 通常先使用兼容的浏览器语音识别；浏览器或平台可能通过自己的服务处理该识别。在 iPhone 和 iPad 网页中，应用只录制一条数据流，避免波形与识别器争抢麦克风，并在停止后转写同一段短录音。浏览器识别不可用、失败或没有返回文字时，才可能把一次尝试临时发送到同源、限速的网关，再由私有 Whisper 服务核对单词。网关只接受来自此来源的准确转写路径，限制文件大小和并发数，不记录或保存音频，并返回 `Cache-Control: no-store`。如果没有转写，波形仍会保留，但不会显示或保存分数。

录音流程使用单一状态锁，防止快速连点创建重复会话；音频启动失败、页面进入后台、录音结束或处理超时时都会关闭音轨与音频上下文。只有检测到足够长且清晰的真实语音才会进入评分。静音、空录音和无法解码的录音会显示具体的重试提示，不再用看似合理的占位声学数据生成误导性分数。
...
| iOS | Capacitor 8 + 原生 AVAudioEngine 录音器 | iPhone 17 Pro 模拟器构建、安装、启动；录音器集成和内嵌 Watch 已通过编译（仍需真机麦克风验证） |
