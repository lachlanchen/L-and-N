...
声学特征、分数、进度和校准均在本地运行。安装版会在操作系统提供时使用原生语音识别。托管的 PWA 会先使用兼容的浏览器语音识别；浏览器或平台可能通过自己的服务处理该识别。仅在没有兼容的浏览器语音识别时，才可能把一次尝试临时发送到同源、限速的网关，再由私有 Whisper 服务核对单词。网关只接受来自该源的准确转写路径，限制文件大小和并发数，不记录或保存音频，并返回 `Cache-Control: no-store`。没有转写时，应用仍然可用。
