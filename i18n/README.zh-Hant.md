...
聲學特徵、分數、進度及校準均在本機運行。安裝版會在作業系統提供時使用原生語音辨識。託管的 PWA 會先使用相容的瀏覽器語音辨識；瀏覽器或平台可能透過自己的服務處理該辨識。僅在沒有相容的瀏覽器語音辨識時，才可能把單次嘗試暫時傳送到同源、限速的閘道，再由私有 Whisper 服務核對單詞。閘道只接受來自該網域的精確轉寫路徑，限制檔案大小和並發數，不記錄或儲存音訊，並回傳 `Cache-Control: no-store`。沒有轉寫時，應用仍然可用。
