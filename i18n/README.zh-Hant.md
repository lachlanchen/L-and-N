...
聲學特徵、分數、進度及校準均在本機運行。安裝版會在系統支援時使用原生語音辨識。託管 PWA 可把單次錄音暫時傳送到同源、限速的閘道，再由私有 Whisper 服務核對單詞。閘道只接受本站精確的轉寫路徑，限制大小和並發，不記錄或儲存音訊，並回傳 `Cache-Control: no-store`。轉寫不可用時仍可練習。
