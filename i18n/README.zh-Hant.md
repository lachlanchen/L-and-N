...
聲學特徵、分數、進度及校準均在本機運行。在 iOS 上，同一條原生音訊引擎資料流同時供波形、本機聲學分析及系統語音辨識使用，應用內部不會再爭用咪高峰。託管的 PWA 通常先使用相容的瀏覽器語音辨識；瀏覽器或平台可能透過自己的服務處理該辨識。在 iPhone 和 iPad 網頁中，應用只錄製一條資料流，避免波形與辨識器爭用咪高峰，並在停止後轉寫同一段短錄音。瀏覽器辨識不可用、失敗或沒有回傳文字時，才可能把單次嘗試暫時傳送到同源、限速的閘道，再由私有 Whisper 服務核對單詞。閘道只接受來自此來源的精確轉寫路徑，限制檔案大小和並發數，不記錄或儲存音訊，並回傳 `Cache-Control: no-store`。如果沒有轉寫，波形仍會保留，但不會顯示或儲存分數。
