...
음향 특징, 점수, 진행도와 보정은 로컬에서 처리합니다. 설치 앱은 운영체제가 제공할 때 네이티브 음성 인식을 사용합니다. 호스팅 PWA는 단어 확인을 위해 녹음을 같은 출처의 속도 제한 게이트웨이를 거쳐 비공개 Whisper로 일시 전송할 수 있습니다. 게이트웨이는 이 사이트의 정확한 경로만 허용하고 크기와 동시 요청을 제한하며 음성을 기록하거나 저장하지 않고 `Cache-Control: no-store`를 반환합니다. 전사가 없어도 연습은 계속됩니다.
