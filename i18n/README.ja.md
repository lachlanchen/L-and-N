...
音響特徴、点数、進捗、較正はローカルで処理します。インストール版は OS が対応するときネイティブ音声認識を利用します。ホスト版 PWA では、単語確認のため録音を同一オリジンの速度制限付きゲートウェイから非公開 Whisper へ一時送信する場合があります。ゲートウェイはこのサイトの正確な経路だけを受け付け、サイズと同時数を制限し、音声を記録・保存せず、`Cache-Control: no-store` を返します。転写がなくても練習できます。
