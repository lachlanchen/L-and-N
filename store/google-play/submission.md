...
- Merchant setup done through Play Console > Settings > Payments profile in the store browser: the existing LazyingArt LLC organization payments profile was linked, the public merchant profile created (Computer Software, statement name `LAZYINGART`, website `lazying.art`, support `contact@lazying.art`), and the Mercury checking account added as the primary payout method. Google will send a test deposit under USD 0.25 within 2-3 days; its amount must be entered under Payments > payment methods to finish verification. US tax info was filed the same day as a **W-8BEN** for the owner (non-US individual, Hong Kong, ITIN, no treaty claim) with a Hong Kong bank statement as proof of address (status In review), and the account group `LazyingArt LLC` was created and **enrolled in the 15 % service fee**.
- Internal testing: `10 (1.0.2)` with Play Billing Library 8.0.0 published 2026-09-20 (bundle SHA-256 `cd82924a1057814ad6201cb0c3a3797e20a68663980b9b597edbb9548f2deea0`). Build 9 with Billing Library 7.1.1 was refused by the console ("must update to at least version 8.0.0") and discarded. Purchases can only be tested on an install from Play (internal test link), not on a sideloaded APK. License-tester lists `L & N license testers` and `L & N phone testers` exist and are enabled on the internal track; on 2026-09-20 the MIX 2S installed build 10 from Play and the unlock card showed the live price, but the purchase sheet did not yet offer the test card, so the purchase itself is still to be confirmed.
