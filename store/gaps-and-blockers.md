...
The first Google Play production release uses a full rollout; staged rollout is reserved for later updates. The App Store price was changed to USD 0.99 on 2026-09-18 and reverted to Free the same day: the Paid Apps Agreement is unsigned, and App Store Connect warned the app could not be made available with a price until it is. Signing it is an account-holder action; the price can then be re-applied through the API. Update (same evening): agreement, bank account, and W-9 are all Active and the 0.99 USD price is back in place. The DSA trader declaration is Active and the EU/EEA storefronts are enabled (175 of 175 territories). Google published 1.0.1 (build 4) to production on 2026-09-18. Version 1.0.1 (build 3) was approved and auto-released on 2026-09-19; the US product page shows 1.0.1 at $0.99. Apple's `itunes.apple.com/lookup` cache still reported the old value for several hours afterwards, so use the product page when checking. Google Play cannot switch this package from free to paid.
