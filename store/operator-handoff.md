...
- All finals 1.0.4 (2026-09-21, latest): web release `78dbab94…` (commit `c02d05f`, adds hear-each-word buttons under the exam answers; native builds 12/9 are at `09aaec6` without them), Android internal `12 (1.0.4)` on both Play apps (production held until the pending reviews finish), iOS build 9 in TestFlight and App Store 1.0.4 waiting for review. 59 pairs; the unverified (weak) words are listed in `docs/research/listening-exam.md`.
