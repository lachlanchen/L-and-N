...
- Chinese scoring (2026-09-23, latest): syllable matching from `src/data/han-homophones.json` (built by `tools/lang/generate_homophones.py`), romanized transcripts, acoustic-only scoring when nothing is recognized, and the iOS `zh-HK` locale fix. Web live; Play internal 16 on both apps; TestFlight builds 14 and 15.
