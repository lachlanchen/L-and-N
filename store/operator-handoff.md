...
- More pairs (2026-09-20, latest): web release `14fff03e…` (commit `3b2a169`), Android internal `8 (1.0.2)`, iOS build 7 uploaded; see `store/artifacts/native-release-1.0.2.json` `pairsRelease`. Studio audio now comes from `tools/audio/synthesize_word_clips.py` (Microsoft neural voices via `edge-tts`, verified with Whisper + onset model); the practice-tab file is the word twice. Public beta still waits on build 4's review.
